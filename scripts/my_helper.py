...
global coords, room_chosen_coord
...
class MyHelper:
    ...
    def __init__(self):
        ...
        # initialize publisher for 360 degrees rotation
        self._joint1Pub = rospy.Publisher('/robot/joint1_position_controller/command', Float64, queue_size=100)
        ...
        # client for move base action server
        self.moveBaseClient = ActionClientHelper('move_base', MoveBaseAction, mutex=self.mutex)
    ...
    def NewLoadOntology(self, path):
        ...
        # \param path that is the path where to save the Ontology
        # \return None
        #
        # This function performs all the clients methods in order to create the ontology and adding
        # all the rooms and corridors, this is made by detcting the ArUco markers informations obtained by the callback
        #
            ...
            if(all_markers_detected):
                ...
                while counter < 2:
                    ...
                    for j in range(len(doors_no_duplicate)):
                        if(client.manipulation.add_ind_to_class(doors_no_duplicate[j], "DOOR") and once == 1):
                            print("Added " , doors_no_duplicate[j]," to DOOR" , end='\r')
                            time.sleep(0.5)
                    ...
                    client.call('DISJOINT', 'IND', 'CLASS', str(individuals_list))
    ...
    def NewLoadOntologyCallback(self, room_msg):
            ...
            ##
            # \brief This function is the callback of NewLoadOntology
            # \param room_msg msg in the callback
            # \return None
            #
            # This function is the callback called every time a msg on the topic /info_room_sm is published
            #
            #
        ...
        if len(roomL) == 7:
            ...
            rospy.loginfo("ALL MARKERS DETECTED!")
    ...
    def UpdateVisitedRoom(self, room):
        ...
        #visitedTime = client.query.dataprop_b2_ind("visitedAt", room)
        print("VISITED TIME LIST: ", visitedTimeList)
        #print("VISITED TIME : ", visitedTime)
    ...
    def FindCoordsRoom(self, fullList, newRoom):
        ##
        # \brief This function obtain the coordinates of the room chosen
        # \param fullList the list of all locations, newRoom the room chosen
        # \return string room object
        for i in range(len(fullList)):
            if newRoom == fullList[i].room:

                return fullList[i]



    def NewMoveRobot(self, newRoom):

        ##
        # \brief This function moves the robot in gazebo
        # \param  newRoom which is the room in which robot should be moved
        # \return None
        #
        # This function takes the coordinates of the room chosen and sends them as a goal point to the move base client
        #

        global room_chosen_coord, fullList

        room_chosen_coord = self.FindCoordsRoom(fullList, newRoom)
        print("Room chosen from coordinates is: ", room_chosen_coord.room)


        goal = MoveBaseGoal()
        goal.target_pose.header.frame_id = "map"
        goal.target_pose.header.stamp = rospy.Time.now()
        goal.target_pose.pose.position.x = room_chosen_coord.x
        goal.target_pose.pose.position.y = room_chosen_coord.y
        goal.target_pose.pose.orientation.w = 1

        print("Room coordinates, x: ", room_chosen_coord.x, " y: ", room_chosen_coord.y)


        # Reset the state of previous stimuli to assure that only the new stimulus are considered.
        self.moveBaseClient.reset_client_states()
    ...
    def VisitingRoom(self):
        ...
        #
        # This function simulates the robot visiting the room rotating arm to 0 position then to 6.28 to have a 360 degrees rotation
        # Always checks at every iteration battery status
        #
        backToZero = 0
        ...
        while(backToZero <= 20):
            try:
                ...
                if self.is_battery_low():
                    ...
                    self._joint1Pub.publish(0.0) #move it to 0 position
                    print('\033[5mSETTING UP FOR SCANNING...\033[0m', end='\r')
                    backToZero += 1
            finally:
                rospy.sleep(1)

        secondsLoop = 0
        while(secondsLoop <= 20):
            try:
                # If the battery is low.
                if self.is_battery_low():
                    print("Battery low arrived ... going to RECHARGING state")
                    ...
                    self._joint1Pub.publish(6.28) #move it to 6.28 position
                    secondsLoop += 1
                    print("Room visited at: ", 5*secondsLoop, " %", end='\r')
